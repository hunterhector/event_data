...
class LemmaJunNombankEventDetector(PackProcessor):
    ...
    def __init__(
        ...
        with open(event_lemma_list_filename, encoding="utf-8") as f:
            ...
            self.event_lemma_list_single = [elm for elm in event_lemmas if
                                            " " not in elm]
            self.event_lemma_list_sequence = [elm.split(" ") for elm in
                                              event_lemmas if " " in elm]
        ...
        self.event_lemma_list_sequence = sorted(self.event_lemma_list_sequence,
                                                key=lambda x: len(x),
                                                reverse=True)
    ...
    def _process(self, pack: DataPack):
        ...
        for ev_seq in self.event_lemma_list_sequence:
            ...
            for sub_idx in sub_idx_list:
                if all([not body_events[idx] for idx in sub_idx]):
                    detected_events_lemma_match.append((body_tokens[
                                                            sub_idx[0]].begin,
                                                        body_tokens[
                                                            sub_idx[-1]].end,
                                                        'lemma', ev_seq))
        ...
        for token, token_is_event in zip(body_tokens, body_events):
            if (not token_is_event) and (
                    token.lemma in self.event_lemma_list_single):
                detected_events_lemma_match.append(
                    (token.begin, token.end, 'lemma', token.lemma))
            if (not token_is_event) and (
                    token.lemma in self.nombank_lemma_list):
                detected_events_lemma_match.append(
                    (token.begin, token.end, 'nombank', token.lemma))

        # event detection result from coling2018-event
        with open(self.coling2018_event_output_path + pack.pack_name + '.ann',
                  'r', encoding="utf-8") as f:
            ann_data = f.read()
        ann = brat_tool.BratAnnotations(ann_data)
        events = ann.getEventAnnotationList()
        for event in events:
            detected_events_coling2018.append((event.textbound.start_pos,
                                               event.textbound.end_pos, 'jun',
                                               event.textbound.text))
        ...
        with open(self.coling2018_event_output_path + pack.pack_name + '.txt',
                  'r', encoding='utf-8') as f:
            txt_data = f.read()
        ...
        for idx in range(len(detected_events) - 1):
            ...
            if int(detected_events[idx + 1][0]) - int(
                    detected_events[idx][1]) <= 2:
                # check dependency 
                for idx_ in range(len(doc) - 1):
                    if doc[idx_].text == detected_events[idx][-1] and doc[
                        idx_ + 1].text == detected_events[idx + 1][-1]:
                        if doc[idx_].dep_ == 'compound' \
                                or (doc[idx_].dep_ == 'prt' or doc[
                            idx_ + 1].dep_ == 'prt') \
                                or (doc[idx_].dep_ == 'dobj' or doc[
                            idx_ + 1].dep_ == 'dobj'):
                            new_event = (detected_events[idx][0],
                                         detected_events[idx + 1][1],
                                         'phrase',
                                         detected_events[idx][-1] + ' ' +
                                         detected_events[idx + 1][-1])
                            replacements.append((detected_events[idx],
                                                 detected_events[idx + 1],
                                                 new_event))
        ...
        for replacement in replacements:
            if replacement[0] not in detected_events:
                logging.info('debug: replacement){} detected_events){}'.format(
                    replacement, detected_events))
                continue
        ...
        for event in detected_events:
            ...
            if type(event[-1]) is list:
                evm = EventMention(pack, event[0],
                                   event[1])  # event[0]: start, event[1]: end
                evm.importance = - 2.0
                ...
                if len(doc) == 1:
                    ...
                    if lemma not in self.reporting_verbs:
                        evm = EventMention(pack, event[0], event[
                            1])  # event[0]: start, event[1]: end
                        evm.event_type = event[2]
                        if lemma in table_word_count and lemma in self.df_table:
                            ...
                            evm.importance = float(
                                '{0:.3g}'.format(tf * math.log(idf)))
